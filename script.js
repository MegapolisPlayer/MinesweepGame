...
function lose() {
	if(isEEEnabled) {
		document.getElementById("name").textContent = "It happens to the best of us :/";
	}
	else {
...
//easter egg (valentine egg?)

async function enableValentineMode() {
	if(IGNORE_EASTER_EGG) return; //tag so it doesnt fall into the wrong hands

	document.documentElement.style.setProperty("background-color", "#7a0242");
	document.documentElement.style.setProperty("color", "#e6b2e8");
	document.getElementById("eenote").innerHTML = "Music: Chopin's Nocturne op.9 no. 1<br>Dedicated to you, Anna 🙂";
	mineImage = await loadImage("rose.png"); //override
	flagImage = await loadImage("roseflag.png"); //override
	nocturne = new Audio("assets/chopinNocturne91.mp3");
	nocturne.volume = 0.7;
	nocturne.play();
	document.getElementById("name").textContent = "Rosesweeper";
	drawField();
	isEEEnabled = true;
}
